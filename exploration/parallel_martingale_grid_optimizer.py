...
class GridOptimizer:
    ...
    def __init__(
            ...
            martingale_count: list,
            martingale_depth: list,
            martingale_cushion: list,
            sizing: list,
    ...
    def __repr__(self) -> str:
        return str(
            dict(
                ...
                martingale_count = self.martingale_count,
                martingale_depth = self.martingale_depth,
                martingale_cushion = self.martingale_cushion,
                sizing = self.sizing,
    ...
    def process_sim(self, 
        ...
        def inputs_list():
            ...
            header = ['sim_name', 'start', 'end', 'init_bal', 'init_trade_size', 'grid_pips', 'tp_grid_count', 'sl_grid_count', 'pyr_grid_count', 'pyr_change_grid_count', 'pyramid_size_factor', 'martingale_count', 'martingale_depth', 'martingale_cushion', 'sizing', 'cash_out_factor', 'trailing_sl', 'gross_bal']
            inputs = [sim_name, start, end, init_bal, init_trade_size, grid_pips, tp_grid_count, sl_grid_count, pyr_grid_count, pyr_change_grid_count, pyramid_size_factor, martingale_count, martingale_depth, martingale_cushion, sizing, cash_out_factor, trailing_sl, gross_bal]
                                            ...
                                            for pgc in self.pyr_grid_count:
                                                for pcgc in self.pyr_change_grid_count:
                                                    for psf in self.pyramid_size_factor:
                                                        for mct in self.martingale_count:
                                                            for mdt in self.martingale_depth:
                                                                for mc in self.martingale_cushion:
                                                                        ...
                                                                        if self.counter >= self.checkpoint:
                                                                            if not self.dummyrun:
                                                                                self.process_sim(
                                                                                    ...
                                                                                    martingale_count=mct,
                                                                                    martingale_depth=mdt,
                                                                                    martingale_cushion=mc,
                                                                                    sizing=s,
                                                                                    cash_out_factor=c,
                                                                                    trailing_sl=tsl
                                                                                )  
