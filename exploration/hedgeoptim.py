...
def set_price(d: Data, i: int, buy_price: float, sell_price: float, reward: float):
...
def set_position_size(d: Data, i: int, rr: float, cushion: float, new_streak: bool=False):
    if new_streak == True:
...
def set_ac_bal(d: Data, i: int, prev_signal: int, reverse: bool):
...
def make_trade(d: Data, i: int, signal: int, risk: float, rr: float, cushion: float, new_streak=False, reverse=True) -> int:
    ...
    set_position_size(d, i=i, rr=rr, cushion=cushion, new_streak=new_streak)
    ...
    if new_streak == True:
        ...
        if i == 0:
            ...
            set_ac_bal(d, i, prev_signal, reverse)
    ...
    if signal == BUY:
        ...
        set_price(d, i, buy_price, sell_price, risk * rr)
...
def open_new_streak(d: Data, i: int, signal: int, risk: float, rr: float, cushion: float, reverse: bool=False) -> int:
    return make_trade(d=d, i=i, signal=signal, risk=risk, rr=rr, cushion=cushion, new_streak=True, reverse=reverse)

def reverse_position(d: Data, i: int, position: int, risk: float, rr: float, cushion: float) -> int:
    signal = BUY if position == SELL else SELL
    return make_trade(d=d, i=i, signal=signal, risk=risk, rr=rr, cushion=cushion, new_streak=False)
    
def no_signal_update_values(d: Data, i: int, position: int):
...
def run_sim(d: Data, sim_name: str, init_signal: int, cushion: float, rr: float, risk: float, streak_trade_limit: int ) -> pd.DataFrame:
    ...
    d.add_columns(name=sim_name, cols=['signal', 'streak_no', 'trade_no', 'position_size', 'cum_position_size', 'buy_price', 'sell_price', 'buy_tp_price', 'sell_tp_price', 'unrealised_pl', 'realised_pl', 'ac_bal', 'margin_used', ])
    ...
    for i in tqdm(range(candles), desc=" Simulating... "):       
        ...
        if i == 0:
            ...
            position = open_new_streak(d=d, i=i, signal=INIT_SIGNAL,  risk=RISK, rr=RR, cushion=CUSHION)
        ...
        # Subsequent candles
            ...
            if ((position == BUY and d.fdata[d.fcols['ask_l']][i] <= d.fdata[d.fcols['sell_price']][i-1]) or \
                ...
                d.fdata[d.fcols['trade_no']][i-1] == STREAK_TRADE_LIMIT:
                # print(i, '#### Margin closeout')
                signal = SELL if position == BUY  else BUY
                position = open_new_streak(d=d, i=i, signal=signal, risk=RISK, rr=RR, cushion=CUSHION, reverse=True)
            ...
            ## From Buy to Sell  
                ...
                position = reverse_position(d=d, i=i, position=position, risk=RISK, rr=RR, cushion=CUSHION)
            ...
            ## Take Profit on long position
                ...
                # print(i, '#### TP Long')
                position = open_new_streak(d=d, i=i, signal=position, risk=RISK, rr=RR, cushion=CUSHION)
            ...
            # Update values when there is no signal
                ...
                # print(i, '#### No signal')
                no_signal_update_values(d=d, i=i, position=position)   
...
def process_sim(d: Data, counter: int, init_signal: int, cushion: float, rr: float, risk: float, streak_trade_limit: int, inputs_list: list, inputs_file: str):
    sim_name = f'sim_{counter}'
    header = ['sim_name', 'init_signal', 'cushion', 'risk', 'rr', 'streak_limit']
    inputs = [sim_name, init_signal, cushion, risk, rr, streak_trade_limit]
